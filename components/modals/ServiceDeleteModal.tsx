...
import { DeleteAdminService } from "../../libs/serviceType";
...
const ServiceDeleteModal = ({
  ...
  const DeleteAccount = async () => {
    if (DeleteText.current?.getValue() === wordToCheck) {
      if (service.id) {
        const response = await DeleteAdminService(service.id);
    ...
    >
      ...
      >
        ...
        >
          ...
          <Text className="font-bold text-3xl">Eliminar Servicio</Text>
          <WarningIcon size={100} color="#cc0000" />
          <Text className="font-thin text-xl text-center">
            Una vez que eliminas el Servicio se eliminaran los datos
            relacionados
          </Text>
